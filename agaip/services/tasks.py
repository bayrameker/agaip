...
from agaip.core.events import publish, TaskStartedEvent, TaskCompletedEvent, TaskFailedEvent
...
async def process_task_async(celery_task, task_id: str, agent_id: str, payload: Dict[str, Any]):
    ...
    try:
        ...
        await publish(TaskCompletedEvent(
            task_id=task_id, 
            agent_id=agent_id, 
            result=result,
...
async def health_check_agents_async():
    ...
    return f"Checked {len(unhealthy_agents)} unhealthy agents, restarted {restarted_count}"

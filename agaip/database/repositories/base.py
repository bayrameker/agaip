...
class BaseRepository:
    ...
    async def get_by_field(self, field_name: str, value: Any) -> Optional[T]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to get {self.model_class.__name__} by {field_name}: {e}"
            )
    ...
    async def soft_delete(self, id: Union[UUID, str, int]) -> bool:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to soft delete {self.model_class.__name__}: {e}"
            )
    ...
    async def exists(self, **kwargs) -> bool:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to check existence of {self.model_class.__name__}: {e}"
            )
    ...
    async def bulk_update(self, instances: List[T], fields: List[str]) -> None:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to bulk update {self.model_class.__name__}: {e}"
            )

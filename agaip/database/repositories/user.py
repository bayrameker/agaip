...
class UserRepository(BaseRepository):
    ...
    async def get_active_users(self, role: Optional[UserRole] = None) -> List[User]:
        ...
        queryset = self.model_class.filter(is_active=True, status=UserStatus.ACTIVE)
    ...
    async def get_locked_users(self) -> List[User]:
        ...
        now = datetime.utcnow()
        return await self.model_class.filter(locked_until__gt=now)
    ...
    async def cleanup_expired_api_keys(self) -> int:
        ...
        users_with_expired_keys = await self.model_class.filter(
            api_key__isnull=False, api_key_expires_at__lt=now
        )
        ...
        if users_with_expired_keys:
            await self.model_class.bulk_update(
                users_with_expired_keys, ["api_key", "api_key_expires_at"]
            )
    ...
    async def get_recent_users(self, days: int = 7, limit: int = 10) -> List[User]:
        ...
        return (
            await self.model_class.filter(created_at__gte=cutoff_date)
            .order_by("-created_at")
            .limit(limit)
        )

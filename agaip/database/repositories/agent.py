...
class AgentRepository(BaseRepository):
    ...
    async def get_unhealthy_agents(self, heartbeat_timeout_minutes: int = 5) -> List[Agent]:
    ...
    async def get_top_performing_agents(self, limit: int = 10) -> List[Agent]:
        ...
        agents = await self.model_class.filter(
            total_tasks_processed__gt=0
        ).order_by("-successful_tasks", "-total_tasks_processed").limit(limit)
    ...
    async def cleanup_inactive_agents(self, days_inactive: int = 30) -> int:
        ...
        inactive_agents = await self.model_class.filter(
            status=AgentStatus.INACTIVE,
            last_heartbeat__lt=cutoff_date
        )
    ...
    async def get_agent_load_distribution(self) -> Dict[str, Any]:
        ...
        for agent in active_agents:
            ...
            load_percentage = (processing_tasks / agent.max_concurrent_tasks * 100) if agent.max_concurrent_tasks > 0 else 0

...
class Task(BaseModel):
    ...
    async def complete_successfully(self, result: Any = None) -> None:
        ...
        await self.save(update_fields=[
            'status', 'completed_at', 'result', 'duration_seconds'
        ])
    ...
    async def fail_with_error(self, error_message: str, error_type: str = None) -> None:
        ...
        await self.save(update_fields=[
            'status', 'completed_at', 'error_message', 'error_type', 'duration_seconds'
        ])
    ...
    async def queue_for_retry(self) -> bool:
        ...
        await self.save(update_fields=[
            'status', 'retry_count', 'queued_at', 'error_message', 'error_type'
        ])
        return True
